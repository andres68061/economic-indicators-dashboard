...
# Define date range
start_date = '1985-01-01'  # Adjusted start date to ensure data availability
...
def load_data():
    ...
    real_gdp = fred.get_series('GDPC1', start_date, end_date).resample('Q').last()
    potential_gdp = fred.get_series('GDPPOT', start_date, end_date).resample('Q').last()
    output_gap = ((real_gdp - potential_gdp) / potential_gdp) * 100
    output_gap.name = 'Output Gap'

    # Unemployment Rate and NAIRU (quarterly)
    unemployment_rate = fred.get_series('UNRATE', start_date, end_date).resample('Q').mean()
    nairu = fred.get_series('NROU', start_date, end_date).resample('Q').mean()
    unemployment_gap = unemployment_rate - nairu
    unemployment_gap.name = 'Unemployment Gap'

    # 10-Year Treasury Yield (quarterly)
    bond_yield_10yr = fred.get_series('DGS10', start_date, end_date).resample('Q').mean()
    bond_yield_10yr.name = '10-Year Treasury Yield'

    # Federal Funds Rate (quarterly)
    federal_funds_rate = fred.get_series('FEDFUNDS', start_date, end_date).resample('Q').mean()
    federal_funds_rate.name = 'Federal Funds Rate'

    # Combine data using an outer join
    data = pd.concat(
        [core_pce, output_gap, unemployment_gap, bond_yield_10yr, federal_funds_rate],
        axis=1,
        join='outer'
    )
...
# Check data types
st.write("Data Types in data_model:")
st.write(data_model.dtypes)

# Check for NaNs and infinite values
st.write("Any NaNs in data_model?", data_model.isnull().any().any())
st.write("Any infinite values in data_model?", np.isinf(data_model).any().any())

# Proceed to scaling if data is clean
if not data_model.isnull().any().any() and not np.isinf(data_model).any().any():
    scaler = MinMaxScaler()
    scaled_data = scaler.fit_transform(data_model)
else:
    st.error("Data contains NaNs or infinite values. Please check the data preprocessing steps.")

# Proceed only if scaling was successful
if 'scaled_data' in locals():
    ...
    # Prediction Section
        ...
        if 'scaled_data' in locals():
            ...
            # Create a full array with the same shape as the scaled data
            y_test_full = np.zeros((y_test.shape[0], scaled_data.shape[1]))
            y_test_full[:, -1] = y_test  # Fill only the target column
            y_pred_full = np.zeros((y_pred.shape[0], scaled_data.shape[1]))
            y_pred_full[:, -1] = y_pred.flatten()  # Fill only the target column
            ...
            **Interpretation:**
            ...
            predicted_full[:, -1] = predicted_rate_scaled.flatten()
